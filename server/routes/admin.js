...
router.post('/accounts', requireAdmin, (req, res) => {
  try {
    ...
    return res.status(400).json({ success: false, message: err.message || 'Failed to create account.' });
  }
});
...
router.put('/accounts/:id', requireAdmin, (req, res) => {
  try {
    ...
    return res.status(400).json({ success: false, message: err.message || 'Failed to update account.' });
  }
});
...
router.patch('/accounts/:id/status', requireAdmin, (req, res) => {
  try {
    ...
    return res.status(400).json({ success: false, message: err.message || 'Failed to update account status.' });
  }
});

// Reset webhook cooldown for specific account
router.post('/accounts/:id/reset-webhook-cooldown', requireAdmin, (req, res) => {
  try {
    const { id } = req.params;
    ...
    return res.status(400).json({ success: false, message: err.message || 'Failed to reset cooldown.' });
  }
});
...
router.delete('/accounts/:id', requireAdmin, (req, res) => {
  try {
    ...
    return res.status(400).json({ success: false, message: err.message || 'Failed to delete account.' });
  }
});
...
router.put('/system-config', requireAdmin, (req, res) => {
  try {
    ...
    return res.json({ success: true, message: 'System configuration updated.', config: updated });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
});
...
router.post('/webhook/send', requireAdmin, async (req, res) => {
  try {
    ...
    if (!message && !fileContent) {
      return res.status(400).json({ success: false, message: 'Message or file content is required.' });
    }

    const accounts = db.getAllAccounts();
    const targetAccounts = accountId === 'all' 
      ? accounts.filter(a => a.webhook_url && a.webhook_url.trim() !== '')
      : accounts.filter(a => a.id === accountId && a.webhook_url && a.webhook_url.trim() !== '');

    if (targetAccounts.length === 0) {
      return res.status(400).json({ success: false, message: 'No accounts with configured Discord webhooks found.' });
    }
    ...
    for (const acc of targetAccounts) {
      try {
        ...
        if (fileContent && fileName) {
          ...
          if (message) {
            formData.append('content', `**[SPACE ADMIN BROADCAST to ${acc.username}]**\n${message}`);
          }
          payload = formData;
        } else {
        ...
        console.error(`Failed to dispatch webhook for ${acc.username}:`, e.message);
        failCount++;
      }
    }
...
router.post('/accounts/:id/devices', requireAdmin, (req, res) => {
  try {
    ...
    const newDevice = db.addDeviceToAccount(req.params.id, { name, type, os, ip, location, status });
    return res.status(201).json({
      success: true,
...
router.put('/accounts/:id/devices/:deviceId', requireAdmin, (req, res) => {
  try {
    ...
    const updated = db.updateDeviceInAccount(req.params.id, req.params.deviceId, { name, type, os, ip, status });
    return res.json({
      success: true,
...
router.patch('/accounts/:id/devices/:deviceId/status', requireAdmin, (req, res) => {
  try {
    const { status } = req.body;
    const updated = db.updateDeviceInAccount(req.params.id, req.params.deviceId, { status });
    return res.json({
      success: true,
...
// Update device customization (wallpaper, accountName, avatar)
router.put('/accounts/:id/devices/:deviceId/customize', requireAdmin, (req, res) => {
  try {
    const { wallpaper, accountName, avatar } = req.body;
    const updated = db.updateDeviceInAccount(req.params.id, req.params.deviceId, { wallpaper, accountName, avatar });
    return res.json({
      success: true,
...
router.post('/accounts/:id/devices/:deviceId/custom-apps', requireAdmin, (req, res) => {
  try {
    const { name, icon, url, action } = req.body;
    const newApp = db.addDeviceCustomApp(req.params.id, req.params.deviceId, { name, icon, url, action });
    return res.status(201).json({
      success: true,
...
router.delete('/accounts/:id/devices/:deviceId/custom-apps/:appId', requireAdmin, (req, res) => {
  try {
    const deleted = db.deleteDeviceCustomApp(req.params.id, req.params.deviceId, req.params.appId);
    return res.json({
      success: true,
...
router.post('/devices/:deviceId/typing', requireAdmin, (req, res) => {
  try {
    ...
    const msg = db.addDeviceTypingMessage(req.params.deviceId, text, from || 'Admin');
    return res.status(201).json({ success: true, message: 'Dispatched to remote device', data: msg });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
});
...
router.delete('/accounts/:id/devices/:deviceId', requireAdmin, (req, res) => {
  try {
    const deleted = db.deleteDeviceFromAccount(req.params.id, req.params.deviceId);
    return res.json({
      success: true,
