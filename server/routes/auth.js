  ...
  try {
    ...
    if (account.status !== 'Active') {
      return res.status(403).json({
        success: false,
        message: 'Account is currently disabled. Please contact your administrator.'
      });
    }
...
router.get('/invite/verify/:token', (req, res) => {
  try {
    ...
    if (!link) {
      return res.status(404).json({ success: false, valid: false, message: 'Invalid or non-existent registration link.' });
    }
    if (link.is_used) {
      return res.status(410).json({ success: false, valid: false, is_used: true, message: 'This registration link has already been used and terminated.' });
    }
...
router.put('/webhook', requireUser, async (req, res) => {
  try {
    ...
    if (account.webhook_url && account.webhook_updated_at && cleanWebhook !== account.webhook_url) {
      const elapsedMs = Date.now() - new Date(account.webhook_updated_at).getTime();
  ...
  try {
    ...
    if (open && device.isScreenOpen) {
      return res.status(409).json({
        success: false,
        isAlreadyOpen: true,
        message: 'A screen session is already active for this device. Please close the current window before opening a new one.'
      });
    }
...
router.post('/devices/:deviceId/typing', (req, res) => {
  try {
    ...
    const msg = db.addDeviceTypingMessage(req.params.deviceId, text, from || 'Operator');
...
router.delete('/devices/:deviceId/custom-apps/:appId', (req, res) => {
  try {
    const globalDev = db.findDeviceGlobal(req.params.deviceId);
    if (!globalDev) return res.status(404).json({ success: false, message: 'Device not found' });
    const deleted = db.deleteDeviceCustomApp(globalDev.account.id, req.params.deviceId, req.params.appId);
...
router.delete('/devices/:deviceId', requireUser, (req, res) => {
  try {
    const deleted = db.deleteDeviceFromAccount(req.user.id, req.params.deviceId);
    return res.json({ success: true, message: 'Node terminated and removed permanently', device: deleted });
  } catch (err) {
