...
import {
  Users, UserPlus, KeyRound, Search, CheckCircle2,
  XCircle, Edit3, Trash2, Power, Eye, EyeOff, Copy, Check,
  LogOut, RefreshCw, AlertCircle, ArrowLeft, Lock, User,
  Link2, Sparkles, Laptop, Server, Plus, Smartphone, Radio, HardDrive,
  ChevronRight, X, Terminal, Monitor
} from 'lucide-react';
...
export default function AdminDashboard({ onLogout, onNavigateApp }) {
  ...
  const [selectedAccountForDevices, setSelectedAccountForDevices] = useState(null);
  ...
  const [devType, setDevType] = useState('device');
  ...
  const fetchAllGlobalDevices = async () => {
    ...
    try {
      ...
      if (devs && devs.length > 0) {
        if (!selectedDeviceForCustom || !devs.some(d => d.id === selectedDeviceForCustom.id)) {
  ...
  const selectDeviceToCustomize = (dev) => {
    ...
    setCustomAccountName(dev.accountName || dev.name || 'Administrator');
  ...
  const handleSaveDevicePersonalization = async (e) => {
    ...
    try {
      ...
      if (updated.wallpaper !== undefined) setCustomWallpaper(updated.wallpaper);
      if (updated.accountName !== undefined) setCustomAccountName(updated.accountName);
      if (updated.avatar !== undefined) setCustomAvatar(updated.avatar);
      setSelectedDeviceForCustom(prev => ({ ...prev, ...updated }));
      setAllGlobalDevices(prev => prev.map(d => d.id === updated.id ? { ...d, ...updated } : d));
      setCustomSuccess('✅ Personalization saved! Reopen the device screen to see the new wallpaper & avatar.');
  ...
  const handleAddCustomAppToDevice = async (e) => {
    ...
    try {
      ...
      sound.playSuccess();
      const updatedApps = [...(selectedDeviceForCustom.customApps || []), newApp];
      setSelectedDeviceForCustom(prev => ({ ...prev, customApps: updatedApps }));
      setAllGlobalDevices(prev => prev.map(d => d.id === selectedDeviceForCustom.id ? { ...d, customApps: updatedApps } : d));
      setNewAppName('');
      setNewAppUrl('');
  ...
  const handleDeleteCustomAppFromDevice = async (appId) => {
    ...
    try {
      ...
      const updatedApps = (selectedDeviceForCustom.customApps || []).filter(a => a.id !== appId);
      setSelectedDeviceForCustom(prev => ({ ...prev, customApps: updatedApps }));
      setAllGlobalDevices(prev => prev.map(d => d.id === selectedDeviceForCustom.id ? { ...d, customApps: updatedApps } : d));
      sound.playSuccess();
    } catch (err) {
  ...
  const handleSendAdminTyping = async (e) => {
    ...
    try {
      await api.sendDeviceTyping(selectedDeviceForCustom.id, adminTypingText.trim(), 'Admin');
      sound.playSuccess();
      setCustomSuccess('Command broadcasted to ' + selectedDeviceForCustom.name + '!');
      setAdminTypingText('');
      setTimeout(() => setCustomSuccess(null), 3000);
    } catch (err) {
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      if (err.message && (err.message.toLowerCase().includes('token') || err.message.toLowerCase().includes('expired') || err.message.toLowerCase().includes('session') || err.message.toLowerCase().includes('auth'))) {
  ...
  const handleGenerateRandomName = async () => {
    ...
    try {
      ...
      setDevName(devType === 'server' ? `SRV-PROD-${randStr(4)}` : `WIN-${randStr(8)}`);
    } finally {
  ...
  const handleAddDevice = async (e) => {
    ...
    sound.playClick();
    try {
      const createdDev = await api.addAccountDevice(selectedAccountForDevices.id, {
      ...
      setDevIp('');
      setShowAddDeviceSection(false);
      setSuccessMsg(`Device "${createdDev.name}" assigned to ${selectedAccountForDevices.username}`);
  ...
  const handleToggleDeviceStatus = async (deviceId, currentStatus) => {
    ...
    sound.playClick();
    try {
      await api.toggleAccountDeviceStatus(selectedAccountForDevices.id, deviceId, newStatus);
      setAccountDevices(prev => prev.map(d => d.id === deviceId ? { ...d, status: newStatus } : d));
      fetchAccounts();
    } catch (err) {
  ...
  const handleDeleteInviteLink = async (id) => {
    ...
    try {
      ...
      setInviteLinks(prev => prev.filter(l => l.id !== id && l.token !== id));
  ...
  const handleCreateAccount = async (e) => {
    ...
    try {
      ...
      sound.playSuccess();
      setSuccessMsg(`Account "${created.username}" created with code: ${created.application_code}`);
      setNewUsername('');
      setNewPassword('');
  ...
  const handleDeleteAccount = async (account) => {
    if (!window.confirm(`Are you sure you want to permanently delete account "${account.username}"?`)) {
      return;
    }
    sound.playClick();
    try {
  ...
  const disabledCount = safeAccounts.filter(a => a.status === 'Disabled').length;
    ...
    <div className="min-h-screen bg-[#050507] text-[#ebebf2] flex flex-col">
      ...
      <header className="h-16 border-b border-white/10 bg-[#0a0a0d] px-6 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold tracking-wider text-white uppercase">SPACE ADMIN</h1>
            <p className="text-[10px] font-mono text-zinc-400">CONTROL & CREDENTIALS MATRIX</p>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-[#0c0c0e] border border-white/10 rounded-xl p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-mono text-zinc-400 uppercase tracking-wider">Total Accounts</p>
          ...
          <div className="bg-[#0c0c0e] border border-white/10 rounded-xl p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-mono text-zinc-400 uppercase tracking-wider">Active Users</p>
          ...
          <div className="bg-[#0c0c0e] border border-white/10 rounded-xl p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-mono text-zinc-400 uppercase tracking-wider">Disabled Users</p>
              <p className="text-2xl font-bold text-zinc-400 mt-1">{disabledCount}</p>
            </div>
            <div className="w-10 h-10 rounded-lg bg-zinc-900 border border-white/10 flex items-center justify-center">
                      ...
                      <tr key={acc.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="py-3.5 px-4 font-mono text-zinc-200">
                          <div className="inline-flex items-center gap-2 bg-zinc-900/90 border border-white/10 px-2.5 py-1 rounded-lg">
                            ...
                            <span className="text-[11px] font-medium tracking-wide">{acc.application_code}</span>
                            <button
                              onClick={() => copyToClipboard(acc.application_code, acc.id)}
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${
                                acc.status === 'Active' ? 'bg-emerald-400' : 'bg-zinc-500'
                              }`}
                            />
                            {acc.status}
                          </span>
                        ...
                        <td className="py-3.5 px-4 text-right">
                          <div className="inline-flex items-center gap-1.5">
                            ...
                            >
                              <Laptop className="w-3.5 h-3.5" />
                              <span className="font-bold">{acc.devices?.length || 0}</span>
                              <span className="text-[9px] text-zinc-500 hidden xl:inline">nodes</span>
                            </button>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-zinc-400">Generates unique Application Code automatically</p>
                  </div>
                </div>
                <button
              ...
              <form onSubmit={handleCreateAccount} className="space-y-4">
                ...
                <div className="p-3 rounded-lg bg-zinc-900/80 border border-white/5 text-[11px] text-zinc-400 flex items-start gap-2">
                  ...
                  <span>A permanent unique code format <code className="text-white font-mono">SPC-XXXX-XXXX-XXX</code> will be assigned upon creation.</span>
                </div>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white uppercase tracking-wider">Edit Account</h3>
                    <p className="text-[11px] text-zinc-400">Modify credentials for {editingAccount.username}</p>
                  </div>
                </div>
                <button
              ...
              <form onSubmit={handleEditAccount} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="w-full h-10 px-3 text-xs rounded-lg bg-zinc-950 border border-white/5 text-zinc-400 font-mono flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-zinc-600 uppercase font-mono">Protected</span>
                  </div>
                </div>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white tracking-wide">Webhook Broadcast</h3>
                    <p className="text-[10px] text-zinc-400 font-mono">Dispatch message or file to Discord webhooks</p>
                  </div>
                </div>
                <button onClick={() => setShowWebhookModal(false)} className="text-zinc-500 hover:text-white text-lg font-mono">&times;</button>
              </div>
              ...
              <form onSubmit={handleSendWebhook} className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {accounts.filter(a => a.webhook_url).map(acc => (
                ...
                <div>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer px-3.5 py-2.5 rounded-lg border border-white/10 hover:border-white/25 bg-zinc-900/90 text-zinc-300 hover:text-white text-xs transition-colors w-full">
                    ...
                    <span className="truncate">{webhookFileName || 'Choose a file to attach...'}</span>
                    <input type="file" onChange={handleWebhookFileUpload} className="hidden" />
                  </label>
                  {webhookFile && (
                    <p className="text-[10px] text-zinc-400 mt-1">File ready: {webhookFileName}</p>
                  )}
                </div>
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-zinc-400">
                      Generate single-use encrypted registration links for new users
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-4 rounded-xl bg-gradient-to-r from-emerald-950/30 via-zinc-900/60 to-zinc-900/40 border border-emerald-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-[11px] text-zinc-400 mt-0.5">
                    Users register with Username & Password. Code is provisioned automatically.
                  </p>
                </div>
                <button
                ...
                {newlyGeneratedUrl && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-mono text-emerald-300/70">Single-Use Only</span>
                    </div>
                    <div className="flex items-center gap-2 bg-black/60 p-2.5 rounded-lg border border-emerald-500/20 font-mono text-[11px] text-zinc-200">
                      <span className="truncate flex-1 select-all">{newlyGeneratedUrl}</span>
                      <button
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  >
                    <RefreshCw className={`w-2.5 h-2.5 ${inviteLoading ? "animate-spin" : ""}`} />
                    <span>Refresh</span>
                  </button>
                </div>
                ...
                <div className="rounded-xl border border-white/5 divide-y divide-white/5 max-h-64 overflow-y-auto bg-zinc-950/40">
                  {inviteLinks.length === 0 ? (
                    <div className="p-6 text-center text-zinc-500 text-xs font-mono">
                      No invite links generated yet. Click "Generate Invite Link" above.
                    </div>
                  ) : (
                    inviteLinks.map((link) => {
                      const isCopied = copiedLinkId === link.id;
                      return (
                        <div key={link.id} className="p-3.5 flex items-center justify-between gap-3 hover:bg-white/[0.02] transition-colors">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-mono font-medium ${link.is_used ? "bg-zinc-800 text-zinc-400 border border-white/5" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${link.is_used ? "bg-zinc-500" : "bg-emerald-400"}`} />
                                {link.is_used ? `Used by @${link.used_by}` : "Active / Ready"}
                              </span>
                            ...
                            <p className="text-[10px] font-mono text-zinc-500 mt-1">
                              ...
                              {link.used_at && ` • Consumed: ${new Date(link.used_at).toLocaleTimeString()}`}
                            </p>
                          </div>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white tracking-wide">Intro Video Manager</h3>
                    <p className="text-[10px] text-zinc-400 font-mono">Select or switch active startup video</p>
                  </div>
                </div>
                <button onClick={() => setShowIntroModal(false)} className="text-zinc-500 hover:text-white text-lg font-mono">&times;</button>
              </div>
              ...
              <div className="space-y-4">
                    ...
                    {[
                      ...
                      >
                        <div>
                          ...
                          <p className="text-[10px] font-mono text-zinc-500">{item.path}</p>
                        </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <label className={`flex-1 h-10 rounded-lg border border-dashed border-purple-500/30 bg-zinc-900/80 flex items-center justify-center gap-2 cursor-pointer hover:bg-purple-500/5 hover:border-purple-500/50 transition-colors ${introUploading ? "opacity-50 pointer-events-none" : ""}`}>
                      <svg className="w-4 h-4 text-purple-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                      </svg>
                      <span className="text-xs text-purple-300">{introUploading ? 'Uploading...' : 'Choose MP4 File'}</span>
                      <input
                        type="file"
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            setSuccessMsg('Video "' + result.filename + '" uploaded successfully!');
                            setTimeout(() => setSuccessMsg(null), 4000);
                          } catch (err) {
                  ...
                  <p className="text-[10px] text-zinc-500 mt-1">Max 200MB. Supported: MP4 and other video formats.</p>
                </div>
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-zinc-400">
                      Provision, simulate, and toggle devices assigned to this user
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-mono text-zinc-400">
                  Total Nodes: <span className="text-white font-bold">{accountDevices.length}</span>
                </span>
                <button
                  onClick={() => {
                ...
                >
                  ...
                  <span>{showAddDeviceSection ? 'Cancel' : 'Add New Node / Device'}</span>
                </button>
              </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="Linux Ubuntu 24.04">Linux Ubuntu 24.04 LTS</option>
              ...
              <div className="space-y-2.5 max-h-[350px] overflow-y-auto pr-1">
                {devicesLoading && accountDevices.length === 0 ? (
                  ...
                  <div className="py-8 text-center rounded-xl border border-dashed border-white/10 p-6 space-y-2">
                    ...
                    <p className="text-xs font-medium text-zinc-400">No devices currently assigned to this account.</p>
                    <p className="text-[11px] text-zinc-600 font-mono">
                      Click "Add New Node / Device" above to connect and provision a device.
                    </p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded-full uppercase flex items-center gap-1 ${
                                isOnline
                                  ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30'
                                  : 'bg-zinc-800 text-zinc-500 border border-white/5'
                              }`}>
                                <span className={`w-1 h-1 rounded-full ${isOnline ? 'bg-emerald-400 animate-pulse' : 'bg-zinc-600'}`} />
                                {dev.status}
                              </span>
                            </div>
                            <p className="text-[10px] text-zinc-400 font-mono mt-0.5 truncate">
                              {dev.os} • IP: {dev.ip} • Ping: {dev.ping}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            ...
                            title={isOnline ? 'Click to make Offline' : 'Click to make Online'}
                          >
            ...
            >
              ...
              <div className="p-4 border-b border-white/10 flex items-center justify-between bg-zinc-900/60 shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-zinc-400 font-mono">
                      Customize wallpapers, user profile, desktop shortcuts, and remote typing
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${devicesHubLoading ? 'animate-spin' : ''}`} />
                  </button>
                  <button
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-12 overflow-hidden">
                ...
                <div className="md:col-span-4 border-r border-white/10 p-3 space-y-2 overflow-y-auto bg-black/40 win11Scroll">
                  <p className="text-[10px] font-mono text-zinc-400 uppercase tracking-wider px-1">SELECT DEVICE</p>
                  {devicesHubLoading && allGlobalDevices.length === 0 ? (
                    <div className="py-8 text-center text-xs text-zinc-500 font-mono">Loading devices...</div>
                  ) : allGlobalDevices.length === 0 ? (
                    <div className="py-8 text-center text-xs text-zinc-500 font-mono">No devices created yet.</div>
                  ) : (
                    allGlobalDevices.map((dev) => {
                      const isSelected = selectedDeviceForCustom && selectedDeviceForCustom.id === dev.id;
                      const isOnline = dev.status === 'Online';
                      return (
                        <div
                          key={dev.id}
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 min-w-0">
                              <span className={`w-2 h-2 rounded-full shrink-0 ${isOnline ? 'bg-emerald-400 animate-pulse' : 'bg-zinc-600'}`} />
                              <span className="font-bold text-xs truncate">{dev.name}</span>
                            </div>
                ...
                {selectedDeviceForCustom ? (
                  <div className="md:col-span-8 p-5 space-y-5 overflow-y-auto win11Scroll">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-xl bg-zinc-900/90 border border-white/10">
                      <div>
                        ...
                        <p className="text-[10px] font-mono text-zinc-400 mt-0.5">
                          Owner: @{selectedDeviceForCustom.accountUsername} • IP: {selectedDeviceForCustom.ip} • Status: {selectedDeviceForCustom.status}
                        </p>
                      </div>

                      <button
                        onClick={() => handleLaunchScreenFromAdmin(selectedDeviceForCustom)}
                    ...
                    <div className="p-4 rounded-xl bg-[#111115] border border-white/10 space-y-3">
                      <h5 className="text-xs font-bold text-cyan-400 uppercase tracking-wider flex items-center gap-2">
                        ...
                        <span>1. System Personalization (خلفية واسم المستخدم والصورة)</span>
                      </h5>

                      <form onSubmit={handleSaveDevicePersonalization} className="space-y-3">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {/* Account Name */}
                          <div>
                            <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">
                              Account Name (اسم المستخدم في الويندوز)
                            </label>
                            <input
                              type="text"
                              value={customAccountName}
                              onChange={(e) => setCustomAccountName(e.target.value)}
                              placeholder="e.g. Administrator / Barof"
                        ...
                        {/* Wallpaper URL & Presets */}
                        <div>
                          <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">
                            Custom Wallpaper (رابط صورة الخلفية)
                          </label>
                          <input
                            type="text"
                            value={customWallpaper}
                            onChange={(e) => setCustomWallpaper(e.target.value)}
                            placeholder="e.g. https://images.unsplash.com/... or preset"
                            className="w-full h-9 px-3 text-xs font-mono rounded-lg bg-zinc-900 border border-white/10 text-white focus:border-cyan-500/50"
                          />
                    ...
                    <div className="p-4 rounded-xl bg-[#111115] border border-white/10 space-y-3">
                      <h5 className="text-xs font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-2">
                        <Plus className="w-3.5 h-3.5" />
                        <span>2. Desktop Custom Apps (إضافة تطبيقات سطح المكتب)</span>
                      </h5>

                      <form onSubmit={handleAddCustomAppToDevice} className="space-y-3 bg-zinc-900/60 p-3 rounded-xl border border-white/5">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                          <div>
                            <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">App Name</label>
                            <input
                              type="text"
                              value={newAppName}
                          ...
                          <div>
                            <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">Icon URL / Preset</label>
                            <input
                              type="text"
                              value={newAppIcon}
                              onChange={(e) => setNewAppIcon(e.target.value)}
                              placeholder="e.g. explorer / chrome / https://..."
                              className="w-full h-8 px-2.5 text-xs font-mono rounded-lg bg-zinc-900 border border-white/10 text-white"
                            />
                          </div>

                          <div>
                            <label className="block text-[10px] font-mono text-zinc-400 uppercase mb-1">Target URL / Command</label>
                            <input
                              type="text"
                              value={newAppUrl}
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 pt-1">
                          <span className="text-[10px] font-mono text-zinc-500">Quick Icons:</span>
                          {[
                      ...
                      <div className="space-y-1.5 pt-1">
                        <p className="text-[10px] font-mono text-zinc-400 uppercase">Installed Custom Apps on Device ({selectedDeviceForCustom.customApps ? selectedDeviceForCustom.customApps.length : 0})</p>
                        {(!selectedDeviceForCustom.customApps || selectedDeviceForCustom.customApps.length === 0) ? (
                          <p className="text-xs text-zinc-600 font-mono py-2">No custom desktop apps added yet.</p>
                        ) : (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {selectedDeviceForCustom.customApps.map((app) => (
                              <div key={app.id} className="p-2.5 rounded-lg bg-zinc-900 border border-white/5 flex items-center justify-between gap-2">
                                <div className="flex items-center gap-2 min-w-0">
                                  <span className="w-2 h-2 rounded-full bg-emerald-400" />
                                  <span className="text-xs font-bold text-white truncate">{app.name}</span>
                                </div>
                                <button
                                  onClick={() => handleDeleteCustomAppFromDevice(app.id)}
                    ...
                    {/* Section 3: Direct Remote Typing Dispatcher */}
                    <div className="p-4 rounded-xl bg-[#111115] border border-white/10 space-y-3">
                      <h5 className="text-xs font-bold text-amber-400 uppercase tracking-wider flex items-center gap-2">
                        <Radio className="w-3.5 h-3.5" />
                        <span>3. Remote Typing & Command Dispatcher (إرسال رسائل وأوامر مباشرة للجهاز)</span>
                      </h5>

                      <form onSubmit={handleSendAdminTyping} className="flex items-center gap-2">
                        <input
                          type="text"
                        ...
                        <button
                          type="submit"
                          disabled={!adminTypingText.trim() || adminTypingLoading}
                        ...
                        >
                          ...
                          <span>{adminTypingLoading ? 'Sending...' : 'Send Live'}</span>
                        </button>
                      </form>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-8 p-12 text-center text-zinc-500 font-mono text-xs flex flex-col items-center justify-center">
                    ...
                    <span>Select a device from the left panel to customize</span>
                  </div>
                )}
              </div>
            </motion.div>
