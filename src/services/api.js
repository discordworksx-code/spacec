...
function getAuthHeaders(isAdmin = false) {
  const token = localStorage.getItem(
    isAdmin ? "space_admin_token" : "space_user_token",
  );
  return {
...
export const api = {
  ...
  async getInviteLinks() {
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data.message || "Failed to fetch invite links");
    return data.links;
  },
  ...
  async createInviteLink() {
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data.message || "Failed to create invite link");
    return data;
  },
  ...
  async deleteInviteLink(id) {
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data.message || "Failed to delete invite link");
    return data;
  },
  ...
  async resetWebhookCooldown(id) {
    const res = await fetch(
      `${API_BASE}/admin/accounts/${id}/reset-webhook-cooldown`,
      {
        method: "POST",
        headers: getAuthHeaders(true),
      },
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data.message || "Failed to reset webhook cooldown");
    return data.account;
  },
  ...
  async updateSystemConfig(config) {
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data.message || "Failed to update system config");
    return data.config;
  },
  ...
  async updateAccountDevice(accountId, deviceId, updates) {
    const res = await fetch(
      `${API_BASE}/admin/accounts/${accountId}/devices/${deviceId}`,
      {
        method: "PUT",
        headers: getAuthHeaders(true),
  ...
  async toggleAccountDeviceStatus(accountId, deviceId, status) {
    const res = await fetch(
      `${API_BASE}/admin/accounts/${accountId}/devices/${deviceId}/status`,
      {
        method: "PATCH",
        headers: getAuthHeaders(true),
        body: JSON.stringify({ status }),
  ...
  async generateDeviceName(type = "device", os = "Windows 11 Pro") {
    const res = await fetch(
      `${API_BASE}/admin/device-name-generator?type=${type}&os=${encodeURIComponent(os)}`,
      {
        headers: getAuthHeaders(true),
      },
    );
  ...
  async updateDevicePersonalization(
    accountId,
    deviceId,
    { wallpaper, deviceName, avatar, type, liveEnabled },
  ) {
    const res = await fetch(
      `${API_BASE}/admin/accounts/${accountId}/devices/${deviceId}/customize`,
      {
        method: "PUT",
        headers: getAuthHeaders(true),
        body: JSON.stringify({
    ...
    if (!res.ok)
      throw new Error(
        data.message || "Failed to update device personalization",
      );
    return data.device;
  },
  ...
  async addDeviceCustomApp(accountId, deviceId, { name, icon, url, action }) {
    const res = await fetch(
      `${API_BASE}/admin/accounts/${accountId}/devices/${deviceId}/custom-apps`,
      {
        method: "POST",
        headers: getAuthHeaders(true),
  ...
  async deleteDeviceCustomApp(accountId, deviceId, appId) {
    const res = await fetch(
      `${API_BASE}/admin/accounts/${accountId}/devices/${deviceId}/custom-apps/${appId}`,
      {
        method: "DELETE",
        headers: getAuthHeaders(true),
      },
  ...
  async uploadImage(file, kind = "viewer") {
    const body = new FormData();
    body.append("image", file);
    const res = await fetch(
      `${API_BASE}/uploads/image?kind=${encodeURIComponent(kind)}`,
      { method: "POST", body },
    );
    const data = await res.json();
  ...
  async uploadDeviceVideo(videoFile, iconFile) {
    const body = new FormData();
    body.append("video", videoFile);
    if (iconFile) body.append("icon", iconFile);
    const res = await fetch(`${API_BASE}/admin/upload-device-video`, {
      method: "POST",
      headers: {
        ...(localStorage.getItem("space_admin_token")
          ? {
              Authorization: `Bearer ${localStorage.getItem("space_admin_token")}`,
            }
          : {}),
      },
      body,
    ...
    const data = await res.json();
    if (!res.ok)
      throw new Error(data.message || "Failed to upload device video");
    return data;
  },

  async sendDeviceMedia(deviceId, payload) {
    const res = await fetch(`${API_BASE}/devices/${deviceId}/media`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || "Failed to send media");
    return data;
  },

  async listFuckVideos() {
    const res = await fetch(`${API_BASE}/fuck/videos`, { cache: "no-store" });
    const data = await res.json();
  ...
  async sendBugReport({ message, imageData, imageName }) {
    const res = await fetch(`${API_BASE}/auth/report-bug`, {
      method: "POST",
      headers: getAuthHeaders(false),
      body: JSON.stringify({ message, imageData, imageName }),
    });
    const data = await res.json();
